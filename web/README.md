...
The preview generator reads the real BCTS raster from `data_layers/adminunits_bcts.zip!bcts.tiff`, using DataLad annex content when present or the ignored local archive fallback at `tmp/shared-data/hectaresbc/data_layers/adminunits_bcts.zip`. It also reads the recovered NRS administrative raster from `data_layers/adminunits_nrsab.zip!nrsab.tiff` to generate a source-derived BC/admin basemap reference. It writes reproducible ignored outputs under `web/data/map_previews/`, including `manifest.json`, `dl_adminunits_bcts/preview.png`, and `context/bc_admin_reference.png`.
