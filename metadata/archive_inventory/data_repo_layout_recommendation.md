...
3. Do not rename, unpack, normalize, or reorganize source ZIP payloads during the initial DataLad import.
...
The `raw/hectaresbc_2022_export/` subtree should preserve the rescued layout exactly below that prefix.
...
Using `raw/hectaresbc_2022_export/` keeps the rescued export intact while leaving room for DataLad repository documentation, metadata, and future derived products.
...
3. Copy or move the rescued archive contents into that prefix without renaming payload files.
...
7. Validate a cold clone can retrieve representative ZIPs with `datalad get`.
